...
int main() {
    ...
    std::ofstream salida;
    salida.open("scaler_300s_FILTERED.csv", std::ios::out);
    ...
    if (files) {
        ...
        while ((file = (TSystemFile *)next())) {
		    ...
		    tree->SetBranchAddress("fCorrectedArrayMeanSigma", &errmean);
		    ...
		    tree->SetBranchAddress("fCorrectedArrayVarianceOfMeanScalers",&arrvar);
        ...
        if (salida.is_open()) {
            ...
            for (size_t i = 0; i < mean300.size(); i++) {
                salida << gpstime300[i] << "," << mean300[i] << "," << errmean300[i]<< ","<<arrvar300[i]<<"," <<stations300[i]<< "," << press300[i]  << std::endl;
            }
